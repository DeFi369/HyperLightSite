...
- `gumroad/profile.html` — the custom Gumroad profile landing page (source of truth; published to hyperlighttech.gumroad.com via the Gumroad CLI — see the 2026-07-02 status entry for the update workflow). Self-contained: fonts embedded as data URIs, no external fetches.
...
**2026-07-02 — STACK LANDED (PRs #3–#9 all merged, LIVE) + Gumroad custom profile page SHIPPED.**
- **Stack landing:** owner authorized merging. Gotcha hit: merging stacked PRs via `gh pr merge --delete-branch` in rapid sequence made GitHub **CLOSE (not retarget)** the dependent PRs #4/#6/#8 when their base branches were deleted. No content lost — recovered by PR **#9** from the chain tip (`blog-round-2` contained every commit) into main; orphaned intermediate branches deleted. **Lesson: merge a stack bottom-up by retargeting each PR to main first, or just merge the tip.** Live-verified after Pages rebuild: canonical → live host ✓, blog (6 articles) ✓, feed ✓, deduped fonts ✓ (all HTTP 200, content grep-checked).
- **Gumroad profile page** (`gumroad/profile.html`, published via `gumroad user page publish`): custom landing page REPLACING the default profile at **https://hyperlighttech.gumroad.com** — "pressboard" design (identity rail + machined-key ladder + product keycaps), fully self-contained (~100KB: Syne+Manrope embedded as data-URI variable fonts), dark default + warm-bone light mode (system pref + Lights on/off toggle, localStorage-persisted), `data-gumroad-field` name/bio interpolation (bio renders `white-space:pre-line` — the live bio is 5 lines), products/posts rendered dynamically from the server-injected `#gumroad-data` JSON (NO checkout elements — links go to product pages), `cleanDescription()` DOMParser pipeline (Gumroad descriptions embed category+name prefix — cut at product name, entities decoded), empty-state fallback, reveal-on-scroll with a **1.2s failsafe** (content can never be stuck invisible if the observer stalls in the sandboxed embed; verified by breaking IO deliberately).
- **Gumroad CLI notes:** installed at `~/.local/bin/gumroad`; auth = OAuth device flow. Sanitizer strips `meta`/`title`/`noscript` (Gumroad owns the outer doc) — final publish report = **0 removals**; data-URI fonts/inline script/JSON block all survive. The live profile hydrates my HTML into a sandboxed SUBFRAME → curl sees an 890-byte shell and headless-chromium screenshots race (virtual-time doesn't drive subframe timers reliably) — verify via the sanitized `custom_html` from the publish response rendered locally. **Update workflow:** edit `gumroad/profile.html` → `gumroad user page preview` (check `.sanitization_report`) → `gumroad user page publish` → revert anytime with `gumroad user page clear --yes`.

**2026-07-02 — `blog-round-2` branch (MERGED via #9).** Blog doubled to 6 articles, one new per guide, same grounded-in-real-chapters rule (sources: Linux ch.9, AI ch.7, DeFi ch.9):
...
**2026-07-01 — `perf-feed` branch (MERGED via #9, LIVE).** Two audit-pass-2 items:
